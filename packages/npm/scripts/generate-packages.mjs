...
const EDLINT_BIN_NAME = "edlint";
const EDLS_BIN_NAME = "edlint_language_server";
const EDLINT_ROOT = resolve(fileURLToPath(import.meta.url), "../..");
const PACKAGES_ROOT = resolve(EDLINT_ROOT, "..");
const REPO_ROOT = resolve(PACKAGES_ROOT, "..");
const MANIFEST_PATH = resolve(EDLINT_ROOT, "package.json");
...
function generateNativePackage(target) {
  const packageName = `@${EDLINT_BIN_NAME}/${target}`;
  const packageRoot = resolve(PACKAGES_ROOT, `${EDLINT_BIN_NAME}-${target}`);
  ...
  const manifest = {
    ...
    ...libc,
    publishConfig: {
      executableFiles: ["edlint", "edlint_language_server"],
    },
  };
  ...
  const oxlintBinSource = resolve(
    REPO_ROOT,
    `${EDLINT_BIN_NAME}-${target}${ext}`
  );
  const oxlintBinTarget = resolve(packageRoot, `${EDLINT_BIN_NAME}${ext}`);

  const oxlsBinSource = resolve(REPO_ROOT, `${EDLS_BIN_NAME}-${target}${ext}`);
  const oxlsBinTarget = resolve(packageRoot, `${EDLS_BIN_NAME}${ext}`);
...
function writeManifest() {
  const manifestPath = resolve(PACKAGES_ROOT, EDLINT_BIN_NAME, "package.json");
...
// NOTE: Must update npm/edlint/bin/edlint
// and npm/edlint/bin/edlint_language_server
const TARGETS = [
  "win32-x64",
  "win32-arm64",
  "linux-x64-gnu",
